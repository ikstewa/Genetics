...
unsigned TextureLibrary::Load(const char *filename)
{
    ...
    if (strstr(filename, ".tga"))
    {
        ...
        if (!img->LoadImage(fName.insert(0, TEXTURE_LIB).c_str()))
        {
            printf("TextureLibrary: ERROR: Failed loading image: %s\n", fName.c_str());
...
void TextureLibrary::Activate(unsigned id)
{
    ...
    {
        printf("TextureLibrary: Invalid texture ID! %ud\n", id);
        return;
    }
    ...
    string textureString = "UNDEFINED";
